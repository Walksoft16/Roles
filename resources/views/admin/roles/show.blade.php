	...
	<div class="col-md-12">
		<div class="box box-primary">
			<div class="box-header">
				...
				<h3 class="box-title">{{trans('Roles::roles.deleteeRole')}}</h3>
			</div>
			<form id="deleteForm" data-parsley-validate="" novalidate="" method="post" action="{{route('roles.destroy',['id'=>$role->id])}}">
                                {{csrf_field()}}
                                {{ method_field('DELETE') }}
                                <fieldset disabled>
                                          ...
                                          <div id="collapse{{$module['id']}}" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
                                            <div class="box-body">
                                                <div class="form-group">
                                                  <div class="checkbox">
                                                    <label>
                                                      <input type="checkbox" name="checkall-{{$module['slug']}}" id="checkall-{{$module['slug']}}" value="" class="checkall flat-red" disabled>
                                                  ...
                                                  <div class="checkbox">
                                                    <label>
                                                      <input disabled class="flat-red" @if(in_array($slug['slug'],$Permission))checked="checked" @endif type="checkbox" name="permits[{{$slug['slug']}}]" id="permits-{{$slug['slug']}}" value="{{$slug['name']}}">
                                                      {{trans($slug['display'])}}
                                                    </label>
                                                  </div>
                                                   @endforeach
